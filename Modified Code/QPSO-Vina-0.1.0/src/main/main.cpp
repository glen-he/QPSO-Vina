...
void do_search(model& m, model ref_m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   const std::string& out_name,
	...
	if(score_only) {
		...
		par(m, ref_m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_birds, w, c1, c2);
		done(verbosity, log);
...
void main_procedure(model& m, const model& ref_m, const boost::optional<model>& ref, // m is non-const (FIXME?)
			     const std::string& out_name,
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref_m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  out_name,
					  corner1, corner2,
			...
			do_search(m, ref_m, ref, wt, prec, c, prec, c, nc,
					  out_name,
					  corner1, corner2,
					  par, energy_range, num_modes,
					  seed, verbosity, score_only, local_only, log, t, weights,num_birds,w,c1,c2);
...
=======================================================================\n";
	...
	try {
		std::string rigid_name, ligand_name, flex_name, real_ligand_name, config_name, out_name, log_name;
		...
		inputs.add_options()
			...
			("real_ligand", value<std::string>(&real_ligand_name), "real_ligand")
		;
		...
		pso.add_options()
			("num_particles", value<int>(&num_birds)->default_value(10), "Number of particles per thread")
		...
		model ref_m = parse_bundle(std::vector<std::string>(1, real_ligand_name));
			
		boost::optional<model> ref;
		done(verbosity, log);

		main_procedure(m, ref_m, ref, 
					out_name,
